...
# --------------------------------------------------------------------------------------
def construct(menu_dict, icon_dict, tree_item, tree, app_config, stack, app, parent):
...
def construct_defaults(
    ...
    validate_icon = QtGui.QIcon(app_config.validate_icon)
    execute_icon = QtGui.QIcon(app_config.execute_icon)
...
def construct_component_menu(
    ...
    for label, func_ in component.user_functions().items():
        menu_dict[f"{label}"] = functools.partial(
            info_refresh_wrapper,
            func_,
            app,
        )
...
# --------------------------------------------------------------------------------------
def info_refresh_wrapper(func_, app):
    result = func_()
    if app.editor_widget:
        app.editor_widget.redraw()
    return result
