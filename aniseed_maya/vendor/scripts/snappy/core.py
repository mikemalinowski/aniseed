...
def groups_in_namespace(namespace):
    groups = []
    for node in cmds.ls(f"{namespace}:*.snapNode", recursive=True, objectsOnly=True):
        groups.append(cmds.getAttr(f"{node}.group"))
...
def get(node, target=None, group=None) -> list[str]:
    ...
    possibilities = [
        attr.split(".")[0]
        for attr in cmds.listConnections(f"{node}.message", type="network", plugs=True) or []
...
@decorators.suspended_viewport
@decorators.undoable
def snap_over_time(group=None, namespace=None, restrict_to=None, start_time=None, end_time=None, key=True):
    """
    This will match all the members of the snap group.
...
def _create_snap_node():
    ...
    """
    snap_node = cmds.createNode("network", name="snap_node")
