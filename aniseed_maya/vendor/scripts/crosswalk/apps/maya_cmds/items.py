...
def get(item_name: str or object) -> object:
    ...
    try:
        ...
        raise RuntimeError(f"Could not find node {item_name}")
...
def get_name(item: object or str) -> str:
    ...
    try:
        return om.MFnDependencyNode(item).name()
...
def set_name(item: object or str, new_name: str) -> str:
    ...
    """
    name = get_name(item)
    return mc.rename(name, new_name)
...
def set_parent(item: object or str, parent: object or str) -> None:
    ...
    """
    if parent:
        mc.parent(get_name(item), parent)
    else:
        mc.parent(get_name(item), world=True)
