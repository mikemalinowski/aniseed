...
class SpaceSwitchComponent(aniseed.RigComponent):
    ...
    def run(self):
        ...
        aniseed.utils.attribute.add_separator_attr(host)

        node_to_drive = aniseed.control.get_classification(
            node_to_drive,
            "org",
        )
    ...
    def add(self, label=None, uuid_=None):
        if not label:
            label = qute.utilities.request.text(
                title="Space Name",
                label="Please give a name for this space",
            )
        ...
        item = qute.QListWidgetItem(label)
        item.uuid_ = uuid_
...
class OptionsBlock(qute.QWidget):

    label_changed = qute.Signal(str)
    ...
    def __init__(self, component, parent=None):
        ...
        self.label = qute.QLineEdit()
        self.target_transform = aniseed.widgets.everywhere.ObjectSelector(component=component)
        self.position_only = qute.QCheckBox()
        self.orientation_only = qute.QCheckBox()
        self.include_scale = qute.QCheckBox()
        self.is_default = qute.QCheckBox()
        ...
        qute.utilities.derive.connectBlind(self.label, self.serialise)
        qute.utilities.derive.connectBlind(self.label, self.label_changed.emit)
        qute.utilities.derive.connectBlind(self.target, self.serialise)
        qute.utilities.derive.connectBlind(self.target_transform, self.serialise)
        qute.utilities.derive.connectBlind(self.position_only, self.serialise)
        qute.utilities.derive.connectBlind(self.orientation_only, self.serialise)
        qute.utilities.derive.connectBlind(self.include_scale, self.serialise)
        qute.utilities.derive.connectBlind(self.is_default, self.serialise)
...
# --------------------------------------------------------------------------------------
# noinspection PyUnresolvedReferences
class SpaceSwitchUi(qute.QWidget):
